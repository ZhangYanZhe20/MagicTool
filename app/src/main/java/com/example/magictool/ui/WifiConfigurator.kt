...
class WifiConfigurator(
    private val host: String = "192.168.54.119",
    ...
    suspend fun getCurrentWifi(): String {
        return try {
            val res = executeCommand("sudo nmcli -t -f active,ssid dev wifi | grep '^yes' | cut -d: -f2")
            res.ifBlank {
                executeCommand("sudo nmcli -t -f name,type con show --active | grep '802-11-wireless' | cut -d: -f1")
            }
        } catch (e: Exception) {
            ""
        }
    ...
    suspend fun scanWifi(): List<String> {
        return try {
            executeCommand("sudo nmcli -t -f ssid dev wifi list")
                .lines()
                .map { it.trim() }
    ...
    suspend fun connectToWifi(ssid: String, passphrase: String? = null): Boolean {
        return try {
            // 1. 先删除旧连接（避免冲突）
            runCatching {
                executeCommand("sudo nmcli con delete ${shellEscape(ssid)}", 10)
            }

            // 2. 开启网卡
            executeCommand("sudo nmcli radio wifi on", 5)

            // 3. 连接命令（带等待、带权限）
            val cmd = if (passphrase.isNullOrEmpty()) {
                "sudo nmcli --wait 10 dev wifi connect ${shellEscape(ssid)}"
            } else {
                "sudo nmcli --wait 10 dev wifi connect ${shellEscape(ssid)} password ${shellEscape(passphrase)}"
            }
    ...
    suspend fun deleteWifi(ssid: String): Boolean {
        return try {
            executeCommand("sudo nmcli con delete ${shellEscape(ssid)}")
            true
        } catch (e: Exception) {
            false
        }
    ...
    suspend fun getSavedWifiList(): List<String> {
        return try {
            executeCommand("sudo nmcli -t -f name,type con show | grep '802-11-wireless' | cut -d: -f1")
                .lines()
    ...
    suspend fun disconnectCurrentWifi(): Boolean {
        return try {
            ...
            // 第二步：直接断开这个连接（100%成功）
            executeCommand("sudo nmcli con down ${shellEscape(currentWifi)}", 10)
            true
        } catch (e: Exception) {
            false
        }
